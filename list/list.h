...
/*Defines the node of a object. Each node contains the elements and pointer to
point at the next node in the list. The last element has NULL as its pointer 
signifying no node after that node. */
struct node {
	elem value;
	struct node *next;
};
...
/* Returns the index of element where it occurs */
